...
{
    const maclist2_t *ia = (const maclist2_t *)a;
    const maclist2_t *ib = (const maclist2_t *)b;
    if (memcmp(ia->addr, ib->addr, 6) > 0)
        return 1;
    else if (memcmp(ia->addr, ib->addr, 6) < 0)
        return -1;
    if (ia->essidlen < ib->essidlen)
        return 1;
    else if (ia->essidlen > ib->essidlen)
        return -1;
    if (memcmp(ia->essid, ib->essid, ib->essidlen) < 0)
        return 1;
    else if (memcmp(ia->essid, ib->essid, ib->essidlen) > 0)
        return -1;
    return 0;
}
...
static int sort_maclist_by_manufacturer(const void *a, const void *b)
{
    ...
    if (memcmp(ia->manufacturer, ib->manufacturer, ib->manufacturerlen) > 0)
        return 1;
    else if (memcmp(ia->manufacturer, ib->manufacturer, ib->manufacturerlen) < 0)
        return -1;
    if (ia->manufacturerlen > ib->manufacturerlen)
        return 1;
    else if (ia->manufacturerlen < ib->manufacturerlen)
        return -1;
    if (memcmp(ia->addr, ib->addr, 6) > 0)
        return 1;
    else if (memcmp(ia->addr, ib->addr, 6) < 0)
...
static int sort_messagelist_by_epcount(const void *a, const void *b)
{
    ...
    if (ia->eapolmsgcount < ib->eapolmsgcount)
        return 1;
    else if (ia->eapolmsgcount > ib->eapolmsgcount)
        return -1;
    return 0;
}
/*===========================================================================*/
...
static int sort_eapmd5hashlist_by_id(const void *a, const void *b)
{
    ...
    if (ia->id < ib->id)
        ...
        return -1;
    if (memcmp(ia->md5request, ib->md5request, EAPMD5_LEN_MAX) > 0)
        return 1;
    else if (memcmp(ia->md5request, ib->md5request, EAPMD5_LEN_MAX) < 0)
        return -1;
    if (memcmp(ia->md5response, ib->md5request, EAPMD5_LEN_MAX) > 0)
        return 1;
    else if (memcmp(ia->md5response, ib->md5response, EAPMD5_LEN_MAX) < 0)
        return -1;
    return 0;
}
/*===========================================================================*/
...
static int sort_eapleaphashlist_by_id(const void *a, const void *b)
{
    ...
    if (ia->id < ib->id)
        ...
        return -1;
    if (ia->leapusernamelen > ib->leapusernamelen)
        return 1;
    if (ia->leapusernamelen < ib->leapusernamelen)
        return -1;
    if (memcmp(ia->leaprequest, ib->leaprequest, LEAPREQ_LEN_MAX) > 0)
        return 1;
    else if (memcmp(ia->leaprequest, ib->leaprequest, LEAPREQ_LEN_MAX) < 0)
        return -1;
    if (memcmp(ia->leapresponse, ib->leapresponse, LEAPRESP_LEN_MAX) > 0)
        return 1;
    else if (memcmp(ia->leapresponse, ib->leapresponse, LEAPRESP_LEN_MAX) < 0)
        return -1;
    else if (memcmp(ia->leapusername, ib->leapusername, ia->leapusernamelen) < 0)
        return -1;
    if (memcmp(ia->leapusername, ib->leapusername, ia->leapusernamelen) > 0)
        return 1;
    else if (memcmp(ia->leapusername, ib->leapusername, ia->leapusernamelen) < 0)
        return -1;
    return 0;
...
static int sort_eapmschapv2hashlist_by_id(const void *a, const void *b)
{
    ...
    if (ia->id < ib->id)
        ...
        return -1;
    if (ia->mschapv2usernamelen > ib->mschapv2usernamelen)
        return 1;
    if (ia->mschapv2usernamelen < ib->mschapv2usernamelen)
        return -1;
    if (memcmp(ia->mschapv2request, ib->mschapv2request, MSCHAPV2REQ_LEN_MAX) > 0)
        return 1;
    else if (memcmp(ia->mschapv2request, ib->mschapv2request, MSCHAPV2REQ_LEN_MAX) < 0)
        return -1;
    if (memcmp(ia->mschapv2response, ib->mschapv2response, MSCHAPV2RESP_LEN_MAX) > 0)
        return 1;
    else if (memcmp(ia->mschapv2response, ib->mschapv2response, MSCHAPV2RESP_LEN_MAX) < 0)
        return -1;
    else if (memcmp(ia->mschapv2username, ib->mschapv2username, ia->mschapv2usernamelen) < 0)
        return -1;
    if (memcmp(ia->mschapv2username, ib->mschapv2username, ia->mschapv2usernamelen) > 0)
        return 1;
    else if (memcmp(ia->mschapv2username, ib->mschapv2username, ia->mschapv2usernamelen) < 0)
        return -1;
    return 0;
